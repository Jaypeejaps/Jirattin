...
function filterchartemp()
{
    ...
    if (isset($_GET['employee_id']) && !empty($_GET['employee_id'])) {
        ...
        $sql = "SELECT * FROM tblticket WHERE employee_id = '$employee_id'";
        $result = mysqli_query($conn, $sql);
...
<body>
    ...
    <div class="main-contentticket">
            ...
            <div class="chartCard">
                <div class="chartBox">
                    <div class="displayMonth">
                        <a href="timeline.php"><button class="btnSwitch" id="timeline-link">Switch to Job</button></a>
                        ...
                        if ($result) {
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                $deadlineDatasets[] = [
                                    ...
                                    'y' => $row['task'] . ': ' . $row['employee_nickname'],
                                    'color' => 'lightblue',
                                    'task' => $row['task'],
                                    'subjob' => $row['subjob'],
        ...
        <script>
            const deadlineDatasets = <?php echo $deadlineDatasetsJson; ?>;
            const durationDatasets = <?php echo $durationDatasetsJson; ?>;
            ...
            function filterDate() {
                ...
                const filteredDatapoints = deadlineDatasets.filter(data => {
                ...
                const filteredDurationDatapoints = durationDatasets.filter(data => {
                    const dataStartDate = new Date(data.x[0]);
                    const dataEndDate = new Date(data.x[1]);
            ...
            function formatChartData(filteredData) {
                ...
                filteredData.forEach((dataItem) => {
                    formattedData.push({
                        ...
                        y: `${dataItem.task}: ${dataItem.employee_nickname}`,
                        color: 'lightblue',
                        task: dataItem.task,
